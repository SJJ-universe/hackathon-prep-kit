...
│   ├─ 06_생활안전_데이터_라이브연결.md   # 카탈로그·프록시·스냅샷으로 실데이터를 화면에
│   └─ 07_데모스캐폴드_영상에셋_실행플랜.md # 다음 작업(#2 데모·#3 영상) 현장/사전 두 플랜
├─ data/
│   ├─ catalog.json    # 생활안전 API 메타(기계가독) — 프록시/검증툴 공용
│   └─ snapshots/      # check-data가 만든 응답 스냅샷(라이브 폴백용)
├─ tools/
│   └─ check-data.mjs  # 대회 전 키·CORS 점검 + 스냅샷 프리페치 (.ps1 래퍼 동봉)
├─ .env.example     # 공공데이터 키 자리(복사해 .env). data.go.kr/safetydata/kakao
└─ scaffold/        # 아이디어 비종속 일반 템플릿. + proxy.mjs/proxy.py(CORS프록시)·data.js(폴백 fetch)
```

- `guides/` 와 `scaffold/` 에는 주제와 무관한 일반 자료만 둔다(도구·데이터·절차·일반 템플릿).
- 두 폴더는 "당일 빠른 출발용 재료"일 뿐, 완성 앱이 아니다.
- 실데이터(생활안전 공공 API) 연결은 [`guides/06_생활안전_데이터_라이브연결.md`](guides/06_생활안전_데이터_라이브연결.md) 참고: `data/catalog.json` + `scaffold/proxy.*`(CORS 우회) + `tools/check-data.mjs`(사전 키점검·스냅샷). 모두 범용 템플릿/지식이라 반입 규칙에 부합한다.
