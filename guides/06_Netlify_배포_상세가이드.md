...
심사위원이 자기 폰으로 QR을 찍어 직접 실행하려면 결과물을 **공개 HTTPS 주소**에 올려야 한다. Netlify는 그 일을 가장 빠르게(드래그 한 번) 해 주는 정적 호스트다. 이 문서는 현재(2026-06) Netlify 화면·명령 기준의 단계별 사용법이다. 빠른 요약은 [03 호스팅·심사위원 시연](03_호스팅_심사위원_시연.md)에 있고, 여기서는 그 1순위 경로(Netlify)를 끝까지 펼친다.
